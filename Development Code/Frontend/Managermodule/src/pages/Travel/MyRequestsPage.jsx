...
<<<<<<< HEAD
  ...
  const calculateStats = useCallback((requests, workflowStatuses) => {
    ...
    requests.forEach(request => {
      ...
      // Check workflow status for submitted requests
  ...
  // Update stats and notify parent component
  const updateStats = useCallback((requests, workflowStatuses) => {
    const stats = calculateStats(requests, workflowStatuses);
...
>>>>>>> upstream/main
      ...
      const errorStatus = { error: 'Failed to load status' };
      const newWorkflowStatuses = {
        ...workflowStatuses,
        [travelRequestId]: errorStatus
      };
      setWorkflowStatuses(newWorkflowStatuses);
      updateStats(requests, newWorkflowStatuses);
      return null;
...
>>>>>>> upstream/main
  ...
  }, [applyFilters, requests, statusFilter, pagination.pageSize, fetchAllWorkflowStatuses]);
  ...
  const handlePageSizeChange = useCallback((newSize) => {
    ...
    fetchAllWorkflowStatuses(paginatedRequests);
  }, [applyFilters, requests, searchTerm, statusFilter, fetchAllWorkflowStatuses]);
