...
public class PolicyQueryServiceImpl implements PolicyQueryService {
    ...
    public PolicyProxyFullDTO getPolicyByEmployee(UUID employeeId, String grade) {
        ...
        Policy p = policyRepository.findByActiveTrue().stream()
                .findFirst()
        ...
        dto.setGradePolicies(
                p.getGradePolicies().stream()
                        .map(gp -> PolicyProxyFullDTO.PolicyGradeDTO.builder()
                                .id(gp.getId())
                                .grade(gp.getGrade())
                                ...
                                .travelModes(
                                        gp.getTravelModes().stream()
                                                .map(tm -> PolicyProxyFullDTO.TravelModeDTO.builder()
                                                        .id(tm.getId())
                                                        .modeName(tm.getModeName())
                                                        .allowedClasses(
                                                                tm.getAllowedClasses().stream()
                                                                        .map(TravelClass::getClassName)
                                                                        .toList() // ✅ Replaced Collectors.toList()
                                                        )
                                                        .build()
                                                )
                                                .toList() // ✅ Replaced Collectors.toList()
