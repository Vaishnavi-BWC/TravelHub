...
import com.bwc.approval_workflow_service.dto.FinanceApprovalActionResponseDTO;
...
public class FinanceApprovalService
        extends AbstractApprovalService<FinanceApprovalActionRequestDTO, FinanceApprovalActionResponseDTO> {

    public FinanceApprovalService(ApprovalWorkflowRepository workflowRepository,
    ...
    @Override
    protected FinanceApprovalActionResponseDTO handleApproval(
            ApprovalWorkflow workflow,
            FinanceApprovalActionRequestDTO request,
            ActorAction action) {
        ...
        switch (request.getActionType()) {
            case APPROVE -> {
                ...
                if (nextStepOpt.isPresent()) {
                    ...
                    message = "Finance approved and forwarded to " + nextStep.getApproverRole() + ".";
                    notifyNextStep(workflow, nextStep.getApproverRole());
                } else {
                    workflow.setStatus("COMPLETED");
                    workflow.setCompletedAt(LocalDateTime.now());
                    message = "Finance approved and workflow completed.";
                }
            }

            case REJECT -> {
                currentStep.setStatus("REJECTED");
                workflow.setStatus("REJECTED_BY_FINANCE");
                workflow.setCompletedAt(LocalDateTime.now());
                message = "Finance rejected the request due to budget constraints.";
            }

            case REQUEST_CLARIFICATION -> {
                workflow.setStatus("CLARIFICATION_REQUESTED_BY_FINANCE");
                message = "Finance requested clarification: " + request.getClarificationRequest();
            }
        ...
        return FinanceApprovalActionResponseDTO.builder()
                ...
                .message(message)
                .budgetApproved(request.getBudgetApproved())
                .expenseCategory(request.getExpenseCategory())
                .withinBudget(request.getWithinBudget())
                .build();
    }
}