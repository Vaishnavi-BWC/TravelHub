...
public class AuthService {
    ...
    public AuthResponse login(AuthRequest req) {
        ...
        log.debug("🟢 [AuthService] Roles fetched from EmployeeService: {}", roles);

        // 🔐 Token includes roles
        String token = jwtUtil.generateToken(user.getEmployeeId(), user.getEmail(), department, roles);

        // 🧩 Log token payload (debug level)
        var parsed = jwtUtil.parseToken(token);
        log.trace("🟢 [AuthService] JWT Payload after encryption: {}", parsed.toJSONObject());
