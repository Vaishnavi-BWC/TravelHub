...
const DashboardPage = () => {
  ...
  // Simple function to fetch MyRequests stats directly
  const fetchMyRequestsStats = async () => {
    try {
      console.log('🔄 Fetching MyRequests stats for dashboard...');
      
      const allRequests = await managerService.getTravelRequestsFilteredByEmployee();
      
      const stats = {
        totalCount: allRequests.length,
        approvedCount: allRequests.filter(req => 
          req.status === 'APPROVED' || req.status === 'approved' || req.status === 'COMPLETED'
        ).length,
        rejectedCount: allRequests.filter(req => 
          req.status === 'REJECTED' || req.status === 'rejected' || 
          (req.status && req.status.startsWith('REJECTED'))
        ).length
      };
      
      console.log('✅ MyRequests stats calculated:', stats);
      setMyRequestsStats(stats);
      
    } catch (error) {
      console.error('❌ Error fetching MyRequests stats:', error);
      // Fallback to personalRequests data if API fails
      setMyRequestsStats({
        totalCount: personalRequests.length,
        approvedCount: personalRequests.filter(req => 
          req.status === 'APPROVED' || req.status === 'approved'
        ).length,
        rejectedCount: personalRequests.filter(req => 
          req.status === 'REJECTED' || req.status === 'rejected'
        ).length
      });
    }
  ...
  // Fetch MyRequests stats when component mounts
  useEffect(() => {
    if (!loading) {
      fetchMyRequestsStats();
    }
  }, [loading]);
  ...
  console.log('🔍 Dashboard Stats Debug:', {
    myRequestsStats,
    personalRequestsLength: personalRequests.length,
    teamRequestsLength: teamRequests.length,
